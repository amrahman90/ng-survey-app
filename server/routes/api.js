...
router.post('/login', (req,res)=>{
    ...
    var sql="SELECT * FROM users WHERE email='"+email+"' and userType=1";
    con.query(sql, function (err, result) {
        ...
        if(result.length > 0)
        {
            ...
            bcrypt.compare(password,result[0].password,function(pwdErr,pwdResponse) {
                ...
                if(pwdResponse == true)
                {
                    ...
                    data={firstName:result[0].firstName, lastName:result[0].lastName,email: result[0].email, image: result[0].image };
                    res.json({result:1,data:data});   // login details are correct and session has been set. user will be login now.
                }
                else
                {
                    res.json({result:0,data:data}); // login details are wrong.
                }
        ...
        else
        {
            ...
            res.json({loginResult:0,data:data}); // login details are wrong.
        }
    });
});
...
// queries to get line chart data.
            ...
            con.query(getDistinctCountriesQuery,(distinctCountriesErr,distinctCountriesRecords)=>{
                ...
                if(distinctCountriesRecords.length > 0)
                {
                    ...
                    _.forEach(distinctCountriesRecords,(distinctCountriesRecord, distinctCountriesRecordIndex)=>{
                        var tempSingleCountryRecord=[];
                        // tempDistinctCountries.push(distinctCountriesRecord.country);
                        var countriesRecordQuery="SELECT COUNT(*) as usersCount, SUBSTRING_INDEX(createdDate, '-', 1) as year, country FROM users WHERE country='"+distinctCountriesRecord.country+"' GROUP BY (SUBSTRING_INDEX(createdDate, '-', 1))";
                        con.query(countriesRecordQuery,(singleCountryErr,singleCountryRecords)=>{
                            // data.lineChartData
                            if(singleCountryRecords.length > 0)
                            {
                                _.forEach(singleCountryRecords,(cRecord,cIndex)=>{

                                    if(d.getFullYear() == cRecord.year || d.getFullYear()-1 == cRecord.year)
                                    tempSingleCountryRecord.push({name:cRecord.year,value:cRecord.usersCount});
                                });
                                data.lineChartData.push({name:singleCountryRecords[0].country,series:tempSingleCountryRecord});
                                if(distinctCountriesRecords.length-1 == distinctCountriesRecordIndex)
                                {
                                    console.log(data);
                                    res.json(data);
                                }
...
router.get('/getProfileDetails',(req,res)=>{
    ...
    //     comments: '',
    //     userType: 1,
    //     createdDate: '2017-12-04T02:34:08.000Z' } ];
    // res.send(a);
    if(req.session.loginSession == true)
    {
...
router.post('/updateProfile',upload.single('image'),(req,res)=>{
    
    var fileStatus=req.file ? req.file.mimetype.split("/")[0] == 'image' ? 1 : 2: 0;
    ...
    var getUserDataQuery="select * from users where id=1";
    con.query(getUserDataQuery,(err,userData)=>{
        if(userData.length > 0)
        {
            // if image extension not exist.
            if(fileStatus == 2)
            {
                return res.json({status:0,result:'Only Image extension type is allowed.'});
            }

            if(fileStatus == 1 && newPassword != null)
            {
                bcrypt.compare(newPassword,userData[0].password,(pwdErr,pwdResponse)=>{
                    console.log(pwdResponse);
                    if(pwdResponse == true)
                    {
                        if(req.body.firstName == userData[0].firstName && req.body.lastName == userData[0].lastName && req.body.email == userData[0].email)
                        {
                            return res.json({status:0, result: 'No change found.'});    
                        }
                        else if(req.body.firstName != userData[0].firstName || req.body.lastName != userData[0].lastName || req.body.email != userData[0].email)
                        {
                            updateData(req.body.firstName, req.body.lastName, req.body.email, req.body.image);
                            return res.json({status:1, result: 'all information but not password updated successfully'});
                            
                        }
                        else
                        {
                            return res.json({status:0, result: 'Same password is not allowed.'});
                        }
                    ...
                    else
                    {
                        bcrypt.hash(req.body.newPassword,saltRounds,function(err,hash){
                            updateData(req.body.firstName, req.body.lastName, req.body.email, req.body.image, hash);
                            return res.json({status:1, result: 'password and file updated successfully'});
                        });
                         
                    }
                });
            }
            else if(fileStatus == 0 && newPassword != null)
            {
                bcrypt.compare(newPassword,userData[0].password,(pwdErr,pwdResponse)=>{
                    console.log(pwdResponse);
                    if(pwdResponse == true)
                    {
                        ...
                        {
                            console.log('file not present, password same but data changed.');
                            updateData(req.body.firstName, req.body.lastName, req.body.email);
                            return res.json({status:1, result: 'all information but not password updated successfully'});
                    ...
                    {
                        bcrypt.hash(req.body.newPassword,saltRounds,function(err,hash){
                            updateData(req.body.firstName, req.body.lastName, req.body.email, undefined, hash);
                            return res.json({status:1, result: 'password but not file updated successfully'});
                        });
                        
                    }
                });
            }
            else if(fileStatus == 1 && newPassword == null)
            {
                updateData(req.body.firstName, req.body.lastName, req.body.email, req.file.filename);
                return res.json({status:1, result: 'file but not password updated successfully'});   
            }
        }
    });


    function updateData(firstName, lastName, email, image,password)
    {
        if(image == undefined && password == undefined)
        {
            var updateQuery="update users set firstName='"+firstName+"',lastName='"+lastName+"',email='"+email+"' where id=1";
            con.query(updateQuery,(UpdateErr,updatedRecord)=>{
                console.log('without image');
            });
        }
        else if(image == undefined && password != undefined)
        {
            var updateQuery="update users set firstName='"+firstName+"',lastName='"+lastName+"',email='"+email+"',password='"+password+"' where id=1";
            con.query(updateQuery,(UpdateErr,updatedRecord)=>{
                console.log('with image and without password');
            });
        }
        else if(image != undefined && password == undefined)
        {
            var updateQuery="update users set firstName='"+firstName+"',lastName='"+lastName+"',email='"+email+"',image='"+image+"' where id=1";
            con.query(updateQuery,(UpdateErr,updatedRecord)=>{
                console.log('with image but without password');
            });
        }
        else
        {
            if(image != undefined && password != undefined)
            {
                var updateQuery="update users set firstName='"+firstName+"',lastName='"+lastName+"',email='"+email+"',image='"+image+"',password='"+password+"' where id=1";
                con.query(updateQuery,(UpdateErr,updatedRecord)=>{
                    console.log('with image with password as well');
                });
            }
        }
